...
static void json_print_case(FILE *stream, const struct FXSubset7ACase *case_data, const struct FXSubset7CRequestRouteResult *result, bool trailing_comma) {
    fputs("    {\n", stream);
    fputs("      \"name\": ", stream);
...
int main(void) {
    ...
    for (size_t index = 0u; index < kFXSubset7ACaseCount; ++index) {
        if (kFXSubset7ACases[index].kind == FXSubset7ARequest) {
            remaining += 1u;
        }
    }

    fputs("{\n", stdout);
    fputs("  \"probe\": \"subset7c_control_request_route\",\n", stdout);
    fputs("  \"format\": \"CORT FX Subset 7C control request route probe\",\n", stdout);
    fputs("  \"results\": [\n", stdout);
