...
static void json_print_case(FILE *stream, const struct FXSubset7ACase *case_data, const struct FXSubset7DResponseProfileResult *result, bool trailing_comma) {
    fputs("    {\n", stream);
    fputs("      \"name\": ", stream);
...
int main(void) {
    ...
    for (size_t index = 0u; index < kFXSubset7ACaseCount; ++index) {
        if (kFXSubset7ACases[index].kind == FXSubset7AResponse &&
            kFXSubset7ACases[index].validation_mode == FXSubset7AValidationAccept) {
            remaining += 1u;
        }
    }

    fputs("{\n", stdout);
    fputs("  \"probe\": \"subset7d_control_response_profile\",\n", stdout);
    fputs("  \"format\": \"CORT FX Subset 7D control response profile probe\",\n", stdout);
    fputs("  \"results\": [\n", stdout);
    ...
    for (size_t index = 0u; index < kFXSubset7ACaseCount; ++index) {
        ...
        if (case_data->kind != FXSubset7AResponse ||
