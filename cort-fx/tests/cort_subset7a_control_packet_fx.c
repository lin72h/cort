...
static void json_print_case(FILE *stream, const struct FXSubset7ACase *case_data, const struct FXSubset7AResult *result, bool trailing_comma) {
    fputs("    {\n", stream);
    fputs("      \"name\": ", stream);
...
int main(void) {
    fputs("{\n", stdout);
    fputs("  \"probe\": \"subset7a_control_packet_decode\",\n", stdout);
    fputs("  \"format\": \"CORT FX Subset 7A control packet probe\",\n", stdout);
    fputs("  \"results\": [\n", stdout);
