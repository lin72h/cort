...
defmodule ReportCaseManifest do
  def main(argv) do
    options =
    ...
    title = CortTooling.required_flag!(options, :title)
    ...
    {report, blockers, _warnings} =
      render_report(actual_path, expected_path, title, actual_label, expected_label)
  ...
  defp render_report(actual_path, expected_path, title, actual_label, expected_label) do
    ...
    verdict =
      if blockers == 0 do
        ...
        "- verdict: blocking issues found; required rows need review before advancing"
      end
...
ReportCaseManifest.main(System.argv())
