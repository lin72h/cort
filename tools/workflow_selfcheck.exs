...
defmodule WorkflowSelfcheck do
  ...
  defp checks(repo_root, artifacts_root, host_os) do
    base_checks = [
      ...
      {"subset7e expected builder", fn -> subset7e_expected_builder_check!(repo_root, artifacts_root) end},
      {"subset7f expected builder", fn -> subset7f_expected_builder_check!(repo_root, artifacts_root) end},
      {"subset7a cases header builder", fn -> subset7a_cases_header_builder_check!(repo_root, artifacts_root) end},
      {"subset7f cases header builder", fn -> subset7f_cases_header_builder_check!(repo_root, artifacts_root) end},
      ...
      {"subset7e compare tool", fn -> subset7e_compare_check!(repo_root) end},
      {"subset7f compare tool", fn -> subset7f_compare_check!(repo_root) end},
      ...
      {"subset7e FX make compare wrapper", fn -> subset7e_make_compare_check!(repo_root, artifacts_root) end},
      {"subset7f FX make compare wrapper", fn -> subset7f_make_compare_check!(repo_root, artifacts_root) end},
  ...
  defp subset7f_cases_header_builder_check!(repo_root, artifacts_root) do
    output_path = Path.join([artifacts_root, "subset7f_notify_transition_cases.h"])
    expected_path = Path.join(repo_root, "cort-fx/tests/generated/subset7f_notify_transition_cases.h")

    run_cmd!(
      Path.join(repo_root, "tools/run_elixir.sh"),
      [
        Path.join(repo_root, "tools/build_subset7f_notify_transition_cases_header.exs"),
        "--scenarios",
        Path.join(repo_root, "fixtures/control/subset7f_notify_state_transition_cases_v1.json"),
        "--output",
        output_path
      ],
      cd: repo_root,
  ...
  defp subset7b_expected_builder_check!(repo_root, artifacts_root) do
    output_path = Path.join([artifacts_root, "subset7b_control_envelope_expected_v1.json"])
    expected_path = Path.join(repo_root, "fixtures/control/subset7b_control_envelope_expected_v1.json")

    run_cmd!(
      Path.join(repo_root, "tools/run_elixir.sh"),
      [
        Path.join(repo_root, "tools/build_subset7b_control_envelope_expected.exs"),
        "--accept-source",
        Path.join(repo_root, "fixtures/control/bplist_packet_corpus_v1.source.json"),
        "--reject-source",
        Path.join(repo_root, "fixtures/control/bplist_packet_rejection_corpus_v1.source.json"),
  ...
  defp subset7b_compare_check!(repo_root) do
    expected_path = Path.join(repo_root, "fixtures/control/subset7b_control_envelope_expected_v1.json")
    ...
    output =
      run_cmd!(
        Path.join(repo_root, "tools/run_elixir.sh"),
        [
          Path.join(repo_root, "tools/compare_subset7b_control_envelope_json.exs"),
          "--fx-json",
          expected_path,
          "--expected-json",
          expected_path
  ...
  defp subset7c_expected_builder_check!(repo_root, artifacts_root) do
    output_path = Path.join([artifacts_root, "subset7c_control_request_route_expected_v1.json"])
    expected_path = Path.join(repo_root, "fixtures/control/subset7c_control_request_route_expected_v1.json")

    run_cmd!(
      Path.join(repo_root, "tools/run_elixir.sh"),
      [
        Path.join(repo_root, "tools/build_subset7c_control_request_route_expected.exs"),
        "--accept-source",
        Path.join(repo_root, "fixtures/control/bplist_packet_corpus_v1.source.json"),
        "--reject-source",
        Path.join(repo_root, "fixtures/control/bplist_packet_rejection_corpus_v1.source.json"),
  ...
  defp subset7c_compare_check!(repo_root) do
    expected_path = Path.join(repo_root, "fixtures/control/subset7c_control_request_route_expected_v1.json")
    ...
    output =
      run_cmd!(
        Path.join(repo_root, "tools/run_elixir.sh"),
        [
          Path.join(repo_root, "tools/compare_subset7c_control_request_json.exs"),
          "--fx-json",
          expected_path,
          "--expected-json",
          expected_path
  ...
  defp subset7e_expected_builder_check!(repo_root, artifacts_root) do
    output_path = Path.join([artifacts_root, "subset7e_notify_client_outcome_expected_v1.json"])
    expected_path = Path.join(repo_root, "fixtures/control/subset7e_notify_client_outcome_expected_v1.json")

    run_cmd!(
      Path.join(repo_root, "tools/run_elixir.sh"),
      [
        Path.join(repo_root, "tools/build_subset7e_notify_client_expected.exs"),
        "--accept-source",
        Path.join(repo_root, "fixtures/control/bplist_packet_corpus_v1.source.json"),
        "--output",
        output_path
  ...
  defp subset7f_expected_builder_check!(repo_root, artifacts_root) do
    output_path = Path.join([artifacts_root, "subset7f_notify_state_transition_expected_v1.json"])
    expected_path = Path.join(repo_root, "fixtures/control/subset7f_notify_state_transition_expected_v1.json")

    run_cmd!(
      Path.join(repo_root, "tools/run_elixir.sh"),
      [
        Path.join(repo_root, "tools/build_subset7f_notify_transition_expected.exs"),
        "--outcomes",
        Path.join(repo_root, "fixtures/control/subset7e_notify_client_outcome_expected_v1.json"),
        "--scenarios",
        Path.join(repo_root, "fixtures/control/subset7f_notify_state_transition_cases_v1.json"),
        "--output",
        output_path
  ...
  defp subset7e_compare_check!(repo_root) do
    expected_path = Path.join(repo_root, "fixtures/control/subset7e_notify_client_outcome_expected_v1.json")
    ...
    output =
      run_cmd!(
        Path.join(repo_root, "tools/run_elixir.sh"),
        [
          Path.join(repo_root, "tools/compare_subset7e_control_notify_json.exs"),
          "--fx-json",
          expected_path,
          "--expected-json",
          expected_path
  ...
  defp subset7f_compare_check!(repo_root) do
    expected_path = Path.join(repo_root, "fixtures/control/subset7f_notify_state_transition_expected_v1.json")
    ...
    output =
      run_cmd!(
        Path.join(repo_root, "tools/run_elixir.sh"),
        [
          Path.join(repo_root, "tools/compare_subset7f_control_notify_transition_json.exs"),
          "--fx-json",
          expected_path,
          "--expected-json",
          expected_path
    ...
    ensure_contains!(output, "- blockers: 0")
    ensure_contains!(output, "- warnings: 0")
    ensure_contains!(output, "`notify.transition.registration.store.check` | match")
  end
  ...
  defp subset1_fx_test_target_check!(repo_root, artifacts_root) do
    ...
    for path <- [
          ...
          Path.join([artifacts_root, "cort-fx", "build", "out", "subset7e_notify_client_outcome_fx.json"]),
    ...
    ensure_contains!(output, "PASS control_notify_outcome_tests")
  ...
  defp shared_fx_handoff_artifacts_check!(repo_root, artifacts_root) do
    shared_pairs = [
      ...
      {"subset7e_notify_client_outcome_fx.json", Path.join([artifacts_root, "cort-fx", "build", "out", "subset7e_notify_client_outcome_fx.json"])},
      {"subset7f_notify_state_transition_fx.json", Path.join([artifacts_root, "cort-fx", "build", "out", "subset7f_notify_state_transition_fx.json"])}
    ]
  ...
  defp subset7b_make_compare_check!(repo_root, artifacts_root) do
    json_path = Path.join([artifacts_root, "cort-fx", "build", "out", "subset7b_control_envelope_fx.json"])
    report_path = Path.join([artifacts_root, "cort-fx", "build", "out", "subset7b_control_envelope_fx_vs_expected_report.md"])

    output =
      run_cmd!(
        "make",
        [
          "compare-subset7b-with-expected",
          "ENVELOPE_EXPECTED_JSON=#{Path.join(repo_root, "fixtures/control/subset7b_control_envelope_expected_v1.json")}"
        ],
        cd: Path.join(repo_root, "cort-fx"),
        env: [{"CORT_ARTIFACTS_ROOT", artifacts_root}],
        expect_exit: 0
    ...
    unless File.exists?(json_path) do
      raise RuntimeError, "missing control-envelope compare input at #{json_path}"
    end
  ...
  defp subset7c_make_compare_check!(repo_root, artifacts_root) do
    json_path = Path.join([artifacts_root, "cort-fx", "build", "out", "subset7c_control_request_route_fx.json"])
    report_path = Path.join([artifacts_root, "cort-fx", "build", "out", "subset7c_control_request_route_fx_vs_expected_report.md"])

    output =
      run_cmd!(
        "make",
        [
          "compare-subset7c-with-expected",
          "REQUEST_ROUTE_EXPECTED_JSON=#{Path.join(repo_root, "fixtures/control/subset7c_control_request_route_expected_v1.json")}"
        ],
        cd: Path.join(repo_root, "cort-fx"),
        env: [{"CORT_ARTIFACTS_ROOT", artifacts_root}],
        expect_exit: 0
    ...
    unless File.exists?(json_path) do
      raise RuntimeError, "missing control request-route compare input at #{json_path}"
    end
  ...
  defp subset7e_make_compare_check!(repo_root, artifacts_root) do
    json_path = Path.join([artifacts_root, "cort-fx", "build", "out", "subset7e_notify_client_outcome_fx.json"])
    report_path = Path.join([artifacts_root, "cort-fx", "build", "out", "subset7e_notify_client_outcome_fx_vs_expected_report.md"])

    output =
      run_cmd!(
        "make",
        [
          "compare-subset7e-with-expected",
          "NOTIFY_OUTCOME_EXPECTED_JSON=#{Path.join(repo_root, "fixtures/control/subset7e_notify_client_outcome_expected_v1.json")}"
        ],
        cd: Path.join(repo_root, "cort-fx"),
        env: [{"CORT_ARTIFACTS_ROOT", artifacts_root}],
        expect_exit: 0
    ...
    unless File.exists?(json_path) do
      raise RuntimeError, "missing notify-client outcome compare input at #{json_path}"
    end
  ...
  defp subset7f_make_compare_check!(repo_root, artifacts_root) do
    json_path = Path.join([artifacts_root, "cort-fx", "build", "out", "subset7f_notify_state_transition_fx.json"])
    report_path = Path.join([artifacts_root, "cort-fx", "build", "out", "subset7f_notify_state_transition_fx_vs_expected_report.md"])

    output =
      run_cmd!(
        "make",
        [
          "compare-subset7f-with-expected",
          "NOTIFY_TRANSITION_EXPECTED_JSON=#{Path.join(repo_root, "fixtures/control/subset7f_notify_state_transition_expected_v1.json")}"
        ],
        cd: Path.join(repo_root, "cort-fx"),
        env: [{"CORT_ARTIFACTS_ROOT", artifacts_root}],
        expect_exit: 0
    ...
    unless File.exists?(json_path) do
      raise RuntimeError, "missing notify transition compare input at #{json_path}"
    end
    ...
    ensure_contains!(output, "- warnings: 0")
    ensure_contains!(File.read!(report_path), "`notify.transition.registration.store.check` | match")
  end
