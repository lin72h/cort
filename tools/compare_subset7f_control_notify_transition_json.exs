...
defmodule CompareSubset7FControlNotifyTransitionJson do
  ...
  def main(argv) do
    ...
    {report, blockers, _warnings} =
      CompareResultSets.compare_paths!(fx_path, expected_path,
        title: "Subset 7F FX vs Expected Notify State Transition Compatibility Comparison",
        ...
        success_verdict: "- verdict: no blocking mismatches found in the shared notify state-transition compatibility surface",
        failure_verdict:
          "- verdict: blocking mismatches found; do not treat FX as semantically aligned for the affected notify state-transition rows"
      )
...
CompareSubset7FControlNotifyTransitionJson.main(System.argv())
