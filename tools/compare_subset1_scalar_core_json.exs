...
defmodule CompareSubset1ScalarCoreJson do
  @blocking_classifications MapSet.new(["required", "required-but-error-text-flexible"])
  @boolean_semantic_fields ["pointer_identity_same", "equal_same_value", "equal_different_value"]
  @numeric_semantic_fields ["length_value"]
  @text_semantic_fields ["primary_value_text", "alternate_value_text"]
  ...
  defp render_report(fx_results, mx_results, fx_label, mx_label) do
    ...
    initial_lines = [
      "# Subset 1A Scalar Core FX vs MX Comparison",
      "",
      "- FX input: `#{fx_label}`",
      "- MX input: `#{mx_label}`",
      ""
    ...
    if all_names == [] do
      ...
      verdict =
        if blockers == 0 do
          "- verdict: no blocking mismatches found in the shared scalar-core comparison surface"
        else
          "- verdict: blocking mismatches found; do not treat FX Subset 1A as semantically aligned for the affected required rows"
        end
  ...
  defp compare_case(name, fx_case, mx_case) do
    ...
    {notes, status, blockers, warnings} =
      ...
      |> maybe_add_semantic_field_notes(blocking_case, fx_case, mx_case, @boolean_semantic_fields)
      |> maybe_add_semantic_field_notes(blocking_case, fx_case, mx_case, @numeric_semantic_fields)
      |> maybe_add_semantic_field_notes(blocking_case, fx_case, mx_case, @text_semantic_fields)
      |> add_diagnostic_field_notes(fx_case, mx_case)

    final_notes =
      if status == "match" and notes == [] do
        ["semantically aligned on shared scalar-core fields"]
      else
        notes
      end
  ...
  defp maybe_add_semantic_field_notes({notes, status, blockers, warnings}, blocking_case, fx_case, mx_case, fields) do
  ...
  defp add_diagnostic_field_notes({notes, status, blockers, warnings}, fx_case, mx_case) do
    updated_notes =
      Enum.reduce(@diagnostic_fields, notes, fn field, acc ->
        fx_value = Map.get(fx_case, field)
        mx_value = Map.get(mx_case, field)
...
CompareSubset1ScalarCoreJson.main(System.argv())
