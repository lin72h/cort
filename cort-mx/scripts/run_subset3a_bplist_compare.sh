...
OUT_DIR=${1:-"$CORT_ARTIFACTS_ROOT/cort-mx/runs/subset3a-mx-bplist-core-compare"}
OUT_DIR=$(absolute_path "$OUT_DIR")
TMP_DIR="${OUT_DIR}.tmp"
COMPARE_TOOL="$CORT_REPO_ROOT/tools/compare_subset3a_bplist_json.exs"
ELIXIR_RUNNER="$CORT_REPO_ROOT/tools/run_elixir.sh"
FX_JSON=${FX_JSON:-${FX_BPLIST_JSON:-"$CORT_REPO_ROOT/subset3a_bplist_fx.json"}}
MX_JSON=${MX_JSON:-${MX_BPLIST_JSON:-"$CORT_ARTIFACTS_ROOT/cort-mx/runs/subset3a-mx-bplist-core/out/subset3a_bplist_core.json"}}
...
rm -rf "$OUT_DIR"
mv "$TMP_DIR" "$OUT_DIR"
printf 'Wrote MX Subset 3A compare artifacts to %s\n' "$OUT_DIR"
exit "$compare_status"
