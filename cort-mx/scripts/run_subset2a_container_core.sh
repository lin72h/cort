...
OUT_DIR=${1:-"$CORT_ARTIFACTS_ROOT/cort-mx/runs/subset2a-mx-container-core"}
OUT_DIR=$(absolute_path "$OUT_DIR")
TMP_DIR="${OUT_DIR}.tmp"
SOURCE_FILE="$CORT_MX_DIR/src/cort_mx_subset2a_container_core.c"
EXPECTATIONS_FILE="$CORT_MX_DIR/expectations/subset2a_container_core_expected.json"
REPORT_TOOL="$CORT_REPO_ROOT/tools/report_case_manifest.exs"
ELIXIR_RUNNER="$CORT_REPO_ROOT/tools/run_elixir.sh"
BIN_NAME="cort_mx_subset2a_container_core"
JSON_NAME="subset2a_container_core.json"
REPORT_TITLE="Subset 2A Container Core Report"
...
(
    ...
    "bin/$BIN_NAME" > "out/$JSON_NAME" 2> "out/subset2a_container_core.stderr"
    cp "out/$JSON_NAME" "out/subset2a_container_core.stdout"
)
...
rm -rf "$OUT_DIR"
mv "$TMP_DIR" "$OUT_DIR"
printf 'Wrote MX Subset 2A container run artifacts to %s\n' "$OUT_DIR"
exit "$report_status"
