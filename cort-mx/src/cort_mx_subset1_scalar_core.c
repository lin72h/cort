...
static struct CaseResult observe_boolean_singleton(
    const char *name,
    CFBooleanRef object,
    bool expectedValue
) {
    ...
    record_type_identity(&result, object);
    ...
    result.equalSameValue = CFEqual(object, object);
    result.hasHashPrimary = true;
    result.hashPrimary = (long)CFHash(object);
    result.hasHashSameValue = true;
    result.hashSameValue = (long)CFHash(object);
...
static struct CaseResult observe_boolean_equality_hash(void) {
    ...
    record_type_identity(&result, trueValue);

    result.hasEqualSameValue = true;
    result.equalSameValue = CFEqual(trueValue, kCFBooleanTrue);
    result.hasEqualDifferentValue = true;
    result.equalDifferentValue = CFEqual(trueValue, falseValue);
    result.hasHashPrimary = true;
    result.hashPrimary = (long)CFHash(trueValue);
    result.hasHashSameValue = true;
    result.hashSameValue = (long)CFHash(kCFBooleanTrue);
    result.hasHashDifferentValue = true;
    result.hashDifferentValue = (long)CFHash(falseValue);
...
static struct CaseResult observe_data_roundtrip(void) {
    ...
    record_type_identity(&result, data);
    ...
    result.hasEqualSameValue = true;
    result.equalSameValue = CFEqual(data, copy);
    result.hasEqualDifferentValue = true;
    result.equalDifferentValue = CFEqual(data, different);
    result.hasHashPrimary = true;
    result.hashPrimary = (long)CFHash(data);
    result.hasHashSameValue = true;
...
static struct CaseResult observe_number_sint32_roundtrip(void) {
    ...
    int32_t value = 42;
    int32_t sameValue = 42;
    int32_t differentValue = 7;
...
static struct CaseResult observe_number_cross_type_equality(void) {
    ...
    int32_t int32Value = 42;
    int64_t int64Value = 42;
    double floatValue = 42.0;
    int64_t roundtripInt64 = 0;
    ...
    record_type_identity(&result, int32Number);
    ...
    result.hasEqualSameValue = true;
    result.equalSameValue = CFEqual(int32Number, int64Number);
    result.hasEqualDifferentValue = true;
    result.equalDifferentValue = CFEqual(int32Number, floatNumber);
    result.hasHashPrimary = true;
    result.hashPrimary = (long)CFHash(int32Number);
    result.hasHashSameValue = true;
    result.hashSameValue = (long)CFHash(int64Number);
    result.hasHashDifferentValue = true;
    result.hashDifferentValue = (long)CFHash(floatNumber);
    ...
    CFRelease(int64Number);
    CFRelease(int32Number);
    return result;
}
...
static void print_results(FILE *stream, const struct CaseResult *results, size_t count) {
    ...
    for (size_t i = 0; i < count; ++i) {
        json_print_result(stream, &results[i], i + 1 < count);
    }
    fputs("  ]\n", stream);
    fputs("}\n", stream);
}
